...
const initializeDOMElements = () => {
  ...
  addTransactionBtnSecondary = document.getElementById(
  ...
  transactionTitleInput = document.getElementById("transactionTitle");
  ...
  transactionDescriptionInput = document.getElementById(
  ...
  budgetAmountElement = document.getElementById("budgetAmount");

  budgetSpentElement = document.getElementById("budgetSpent");

  budgetRemainingElement = document.getElementById("budgetRemaining");

  budgetPercentageElement = document.getElementById("budgetPercentage");

  budgetProgressBar = document.getElementById("budgetProgressBar");

  budgetStatusElement = document.getElementById("budgetStatus");

  //    OPTIONAL ANALYTICS CANVASES

  incomeExpenseChartCanvas = document.getElementById("incomeExpenseChart");

  expenseCategoryChartCanvas = document.getElementById("expenseCategoryChart");

  monthlyOverviewChartCanvas = document.getElementById("monthlyOverviewChart");
};
...
const saveTransactions = async () => {
  try {
    await localStorage.setItem(STORAGE_KEY, JSON.stringify(transactions));
  } catch (error) {
    console.error("Unable to save transactions:", error);

    alert("Your transactions could not be saved.");
  }
};
...
const loadBudget = async () => {
  try {
    const savedBudget = await localStorage.getItem(BUDGET_STORAGE_KEY);
...
const saveBudget = async () => {
  try {
    await localStorage.setItem(BUDGET_STORAGE_KEY, String(monthlyBudget));
  } catch (error) {
    console.error("Unable to save budget:", error);

    alert("Your budget could not be saved.");
  }
};
...
const handleTransactionSubmit = (event) => {
  ...
  const type = selectedType ? selectedType.value : "income";

  const title = transactionTitleInput ? transactionTitleInput.value.trim() : "";

  const amount = transactionAmountInput
    ? Number(transactionAmountInput.value)
    : 0;

  const category = transactionCategoryInput
    ? transactionCategoryInput.value
    : "";

  const date = transactionDateInput ? transactionDateInput.value : "";

  const description = transactionDescriptionInput
...
const openDeleteModal = (transactionId) => {
  ...
  transactionBeingDeleted = transactionId;
...
const confirmDeleteTransaction = () => {
  ...
  transactions = transactions.filter(
    (transaction) => transaction.id !== transactionBeingDeleted,
  );
...
const updateBudgetDisplay = () => {
  ...
  if (monthlyBudget <= 0) {
    budgetAmountElement.textContent = formatCurrency(0);

    budgetSpentElement.textContent = formatCurrency(0);

    budgetRemainingElement.textContent = formatCurrency(0);

    budgetPercentageElement.textContent = "0% used";

    budgetProgressBar.style.width = "0%";

    budgetProgressBar.className = "budget-progress-bar";

    budgetStatusElement.textContent = "No budget set";
  ...
  budgetAmountElement.textContent = formatCurrency(monthlyBudget);

  budgetSpentElement.textContent = formatCurrency(spent);

  budgetRemainingElement.textContent = formatCurrency(Math.max(remaining, 0));
  ...
  if (percentage < 80) {
    budgetStatusElement.textContent = "You're on track";

    budgetStatusElement.classList.add("safe");
  } else if (percentage < 100) {
    budgetStatusElement.textContent = "Approaching limit";

    budgetStatusElement.classList.add("warning");

    budgetProgressBar.classList.add("warning");
  } else {
    budgetStatusElement.textContent = "Budget exceeded";
...
const updateExpenseCategoryChart = () => {
  if (!expenseCategoryChartCanvas || typeof Chart === "undefined") {
    return;
  }
...
const updateMonthlyOverviewChart = () => {
  if (!monthlyOverviewChartCanvas || typeof Chart === "undefined") {
    return;
  }
  ...
  monthlyOverviewChart = new Chart(monthlyOverviewChartCanvas, {
    type: "bar",
...
const initializeEventListeners = () => {
  ...
  if (addTransactionBtnSecondary) {
    addTransactionBtnSecondary.addEventListener("click", function () {
      openTransactionModal();
    });
  }
  ...
  document.addEventListener("keydown", function (event) {
    ...
    if (budgetModal && budgetModal.classList.contains("show")) {
      closeBudgetModal();
    }
  });
};
